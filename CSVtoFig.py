...
dfall = pd.read_csv(Filename,encoding='cp932',header = 43)
...
for i in range(len(peaks)):
    if i == 0:
        data_li.append(dfall.iloc[:round(np.median(peaks))])
    elif i == 1:
...
for i in range(len(peaks)):
    ...
    min_depth = round(result.params.valuesdict()['center']+result.params.valuesdict()['sigma']*4) #95%
...
axes[1,1].scatter(peak_data_li[0]["DO [mg/l]"], peak_data_li[0]["深度 [m]"])
axes[1,1].scatter(peak_data_li[1]["DO [mg/l]"], peak_data_li[1]["深度 [m]"])
